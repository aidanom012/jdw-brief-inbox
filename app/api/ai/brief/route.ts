...
function jsonError(message: string, status: number, issues?: string[]) {
  return NextResponse.json({ ok: false, message, issues }, { status });
}

export function GET() {
  return jsonError("Use POST to generate an AI brief.", 405);
}

export async function POST(request: NextRequest) {
  const role = readSessionRole(request.cookies.get(SESSION_COOKIE_NAME)?.value);
  if (!role) {
    return jsonError("Log in before using the AI parser.", 401);
  }
  ...
  const rawBrief = body && typeof body === "object" && "rawBrief" in body
  ...
  if (rawBrief.length > MAX_RAW_AI_BRIEF_LENGTH) {
    return jsonError("Raw brief is too large for the AI parser.", 400);
  }

  try {
    const result = await generateAiBrief(rawBrief);
